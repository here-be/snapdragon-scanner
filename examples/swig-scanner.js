/**
 * Token types from https://github.com/paularmstrong/swig
 */
...
const scanner = new Scanner(str, { rules });
console.timeEnd('load');
const tokens = scanner.scanWhile(tok => {
  if (tok && tok.type) {
    tok.value = tok.match[tok.idx || 0].replace(/\s*$/, '');
    // console.log(tok.type)
    if (tok.type.replace && tok.type.replace[tok.value]) {
      tok.value = tok.type.replace[tok.value];
    }
  }
  return !scanner.eos();
});
